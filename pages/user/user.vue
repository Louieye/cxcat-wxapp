 <template>
	<view>
		<!--主体-->
		<view class="meMain">
			<image src="cloud://lyj-app.6c79-lyj-app-1301672818/icon/meBg.png"></image>
			<view class="mePosition">
				<view class="meMainBox">
					...
					<view class="meHead">
						...
						<view><text class="meHeadName">{{ nickName }}</text></view>
						<van-button class="meButton" type="primary" v-if="buttonShow" open-type="getUserInfo" @getuserinfo="getUserInfo">授权登录</van-button>
					</view>
					...
					<view class="meOverBg">
						...
						<van-cell-group v-for="item in appoinList">
						  <van-cell 
						  :title="item.title"
						  :value="item.date"
						  is-link
...
<script>
	...
	export default{
		...
		onShow() {
			...
			db.collection('appointment').where({_openid:openid}).get().then(res => {
				// res.data 包含该记录的数据
				this.appoinList = res.data;
				console.log(res);
			});
		},
		methods:{
...
<style lang="scss" scoped>
	...
	.meButton{ position: absolute;left: 50%;transform: translate(0,20%);}
